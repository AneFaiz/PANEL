...
router.get("/instance/:id/automations", async (req, res) => {
    ...
    // Add egg-specific automation options
    const automationOptions = {
        powerActions: ['start', 'stop', 'restart', 'kill'],
        scriptActions: [],
        customCommands: []
    };

    // Add egg-specific scripts if available
    if (eggData?.Scripts) {
        automationOptions.scriptActions = Object.entries(eggData.Scripts).map(([name, script]) => ({
            name,
            script,
            description: `Run ${name} script from egg configuration`
        }));
    }

    // Add egg-specific custom commands if available
    if (eggData?.CustomCommands) {
        automationOptions.customCommands = eggData.CustomCommands.map(cmd => ({
            name: cmd.name,
            command: cmd.command,
...
function executeWorkflow(instanceId) {
    ...
    if (workflow) {
        ...
        blocks
            ...
            .forEach(block => {
                executeScriptAction(instanceId, block.meta.selectedValue)
                    .then(success => {
                        if (success) {
                            const webhookBlock = blocks.find(b => b.type === 'webhook');
                            if (webhookBlock) {
                                sendWebhookNotification(webhookBlock.meta.inputValue, `Successfully executed script: ${block.meta.selectedValue}`);
                            }
                        }
                    });
            });
        ...
        blocks
            ...
            .forEach(block => {
                executeCustomCommand(instanceId, block.meta.selectedValue)
                    .then(success => {
                        if (success) {
                            const webhookBlock = blocks.find(b => b.type === 'webhook');
                            if (webhookBlock) {
                                sendWebhookNotification(webhookBlock.meta.inputValue, `Successfully executed command: ${block.meta.selectedValue}`);
                            }
                        }
                    });
            });
...
async function executeScriptAction(instanceId, scriptName) {
    try {
        const instance = await db.get(instanceId + '_instance');
        const eggs = await db.get('eggs') || [];
        const eggData = instance.eggData || eggs.find(e => e.Id === instance.imageData?.Id);

        if (!eggData?.Scripts?.[scriptName]) {
            log.error(`Script ${scriptName} not found in egg configuration`);
            return false;
        }

        const scriptContent = eggData.Scripts[scriptName];
        const response = await axios.post(
            `http://${instance.Node.address}:${instance.Node.port}/instances/${instance.ContainerId}/script`,
            {
        ...
        log.error('Error executing script action:', error.message);
        return false;
    }
}

async function executeCustomCommand(instanceId, commandName) {
    try {
        const instance = await db.get(instanceId + '_instance');
        const eggs = await db.get('eggs') || [];
        const eggData = instance.eggData || eggs.find(e => e.Id === instance.imageData?.Id);

        const command = eggData?.CustomCommands?.find(cmd => cmd.name === commandName);
        if (!command) {
            log.error(`Command ${commandName} not found in egg configuration`);
            return false;
        }

        const response = await axios.post(
            `http://${instance.Node.address}:${instance.Node.port}/instances/${instance.ContainerId}/command`,
            {
        ...
        log.error('Error executing custom command:', error.message);
        return false;
    }
}
